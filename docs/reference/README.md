# Reference Documentation

Complete reference guides for tools and versions.

This directory contains detailed reference documentation and complete guides.

## Contents

- **USAGE_GUIDE.md** - Complete guide for using Converter, Generator, and Runner tools
  - GUI and command-line usage
  - All options and parameters
  - Practical examples and workflows
  - Performance tips and troubleshooting

- **VERSION_HISTORY.md** - Detailed version history from v1.0 to v2.0
  - Version features and changes
  - Breaking changes and migration guides
  - Roadmap for future versions
  - Feature comparison table

## How to Use

### For Tool Usage
→ See **USAGE_GUIDE.md**

Examples:
- [Data Converter Guide](USAGE_GUIDE.md#data-converter)
- [Instance Generator Guide](USAGE_GUIDE.md#instance-generator)
- [Test Runner Guide](USAGE_GUIDE.md#test-runner)
- [Batch Workflows](USAGE_GUIDE.md#batch-workflow-example)

### For Version Information
→ See **VERSION_HISTORY.md**

Examples:
- [v2.0.0 Features](VERSION_HISTORY.md#v200---multi-tool-framework-april-2026)
- [Migration Guide](VERSION_HISTORY.md#migration-guide-v140--v200)
- [Future Roadmap](VERSION_HISTORY.md#future-roadmap)

## Related Documentation

- [../getting-started/GETTING_STARTED.md](../getting-started/GETTING_STARTED.md) - Installation
- [../overview/PROJECT_OVERVIEW.md](../overview/PROJECT_OVERVIEW.md) - Project information
- [../guides/TROUBLESHOOTING.md](../guides/TROUBLESHOOTING.md) - Troubleshooting
